FROM alpine:3.2
MAINTAINER The Prometheus Authors <prometheus-developers@googlegroups.com>

ENV GOPATH /go
COPY . /go/src/github.com/prometheus/prometheus

RUN apk add --update -t build-deps go git mercurial
RUN apk add -u musl && rm -rf /var/cache/apk/*
RUN go get github.com/tools/godep
WORKDIR /go/src/github.com/prometheus/prometheus/cmd/prometheus
RUN $GOPATH/bin/godep restore && go get -d
RUN go build -ldflags " \
            -X main.buildVersion  $(cat ../../VERSION) \
            -X main.buildRevision $(git rev-parse --short HEAD) \
            -X main.buildBranch   $(git rev-parse --abbrev-ref HEAD) \
            -X main.buildUser     root \
            -X main.buildDate     $(date +%Y%m%d-%H:%M:%S) \
            -X main.goVersion     $(go version | awk '{print substr($3,3)}') \
        " -o /bin/prometheus
WORKDIR /go/src/github.com/prometheus/prometheus/tools/rule_checker 
RUN go build -o /bin/rule_checker
WORKDIR /go/src/github.com/prometheus/prometheus
RUN mkdir -p /etc/prometheus
RUN mv ./documentation/examples/prometheus.yml /etc/prometheus/prometheus.yml
RUN mv ./console_libraries/ ./consoles/ /etc/prometheus/
RUN rm -rf /go
RUN apk del --purge build-deps

EXPOSE     9090
VOLUME     [ "/prometheus" ]
WORKDIR    /prometheus
ENTRYPOINT [ "/bin/prometheus" ]
CMD        [ "-config.file=/etc/prometheus/prometheus.yml", \
             "-storage.local.path=/prometheus", \
             "-web.console.libraries=/etc/prometheus/console_libraries", \
             "-web.console.templates=/etc/prometheus/consoles", \
             "-log.level=debug"]
